...
##회고 
- 숫자들간 공통 부분을 뽑아 숫자들을 조합해서 만들고 최대값을 구하는 문제
- 숫자의 조합이니 combinations 함수를 사용했는데 시간 초과
- counter로 중복 있는 거만 뽑으려니 중복 2개 있는 걸 반영 못함 , intersaction 활용해보렸는데 잘 안됨 
- 이중 반복문 돌리고 중복 요소 따로 리스트에 추가 후 삭제하려니 인덱스 길이가 또 고민.. 
많은 시도를 하다가 결국 구글링했는데  X,Y자체를 비교하지 않고 0~9 숫자고 기준을 잡아 반복문을 돌리는 아이디어가 있었다..! sort 사용을 안해도 되고 시간 단축도 할 수 있었던 방법.. 내가 포기했던 counter와 intersaction을 활용해서 푼 코드도 있었다. 다양한 풀이법 고민 많이 해보자
