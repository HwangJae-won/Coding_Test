...
## 회고 
수행 속도와 최적 코드의 중요성을 알게된 문제. 약수 구하기 문제였는데, 나눠서 0이 나오는 케이스를 약수로 생각하고 풀었더니 숫자가 큰 케이스에서는 넘어가기 못해서 시간 초과가 났다. 약수랑 소수 문제는 빈출 유형이라고 하니 잘 정리해둘필요가 있는 것 같다. 루트를 써서 속도를 좀더 줄여봤는데 통과되는 속도를 보니 더 줄일 여지가 있는 듯하다. 다른 코드 확인하면서 정리해둘 필요 있어보임 
