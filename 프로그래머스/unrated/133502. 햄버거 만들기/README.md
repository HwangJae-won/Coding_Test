...
## 회고 
앞에서부터 리스트를 읽어와 조건을 만족하는지 확인하고, 만족하면 횟수 증가 후 삭제하도록 만들었다. temp[-4:] 슬라이싱을 안해서 몇몇 케이스에서 오류가 났었는데, 슬라이싱을 해줘야 [1,2,3,1]의 리스트와 길이가 맞아 비교가 가능했다.난 del을 써서 리스트를 없애줬는데, pop을 이용할 수도 있는 것 같다
