...
##회고
프로그래머스 카카오 블라인드리쿠르팅: 신고 결과 받기 

역시 카카오네,, 어렵다 중복 신고는 제거하고, k번 이상 신고당하면 아웃, 신고자가 받게 될 메일 수를 반환하는 문제였다. 문자열을 split해서 담는 것까지는 어렵지 않았는데 리스트를 활용해서 풀려니 한계가 많아 딕셔너리로 접근해보려했다. 딕셔너리로 해보려니 add같이 추가하는 함수가 없어서 반복문을 돌리기 어려웠다. 검색해보니 set을 활용할 수 있었고, defaultdict이라는 함수로 기본 딕셔너리를 만들 수 있었다. 기본 딕셔너리 생성 후 반복문 돌리는 거 좋은 아이디어 같다 기억해두자! 이렇게 딕셔너리 구성하면 이후 코드는 어렵지 않았다.
### 성능 요약
